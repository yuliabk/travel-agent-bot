...
const travelStyleOptions = [
  { value: 'חוף ים', label: 'חוף ים', icon: Waves }, { value: 'ערים', label: 'ערים', icon: Building2 }, { value: 'טבע', label: 'טבע', icon: Trees }, { value: 'תרבות', label: 'תרבות', icon: Landmark }, { value: 'הרפתקאות', label: 'הרפתקאות', icon: Mountain }, { value: 'ספא', label: 'ספא ורוגע', icon: Sparkles },
]
export function StepPreferences({ form, updateForm, onNext, onBack }: Props) {
  const styles = form?.travelStyles ?? []
  const toggleStyle = (style: string) => updateForm({ travelStyles: styles.includes(style) ? styles.filter((s: string) => s !== style) : [...styles, style] })
  return (
