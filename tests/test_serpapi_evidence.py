                ...
                "flights": [
                    {
                        "airline": "Example Air",
                        "flight_number": "EA100",
                        "departure_airport": {"id": "TLV", "time": "10:00"},
                        "arrival_airport": {"id": "FCO", "time": "13:45"},
                    }
                ],
            }
        ],
...
def test_missing_search_id_cannot_be_called_verified():
    data = {
        "search_parameters": {"currency": "USD"},
        "best_flights": [{"price": 510, "flights": []}],
    }
    record = normalize_flights_response(data)[0]
        ...
        "properties": [
            {
                ...
                "overall_rating": 4.5,
                "rate_per_night": {"lowest": "€ 123"},
                "prices": [
                    {"source": "Booking.com", "rate_per_night": {"lowest": "€ 130"}},
                    {"source": "Agoda", "rate_per_night": {"lowest": "€ 125"}},
                ],
            }
        ],
    }
...
def test_missing_price_is_explicit_and_not_inferred():
    data = {
        "search_metadata": {"id": "hotel-search"},
        "search_parameters": {"currency": "USD"},
        "properties": [{"name": "No Price Hotel", "property_token": "x"}],
    }
    record = normalize_hotels_response(data)[0]
...
def test_build_evidence_pack_combines_categories_under_request():
    pack = build_evidence_pack(
        "req_123",
        flights_response={
            "search_metadata": {"id": "f"},
            "search_parameters": {"currency": "USD"},
            "best_flights": [{"price": 200, "flights": []}],
        },
        hotels_response={
            "search_metadata": {"id": "h"},
            "search_parameters": {"currency": "USD"},
            "properties": [{"property_token": "p", "rate_per_night": {"lowest": "$80"}}],
        },
    )
