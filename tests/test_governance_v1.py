...
def verified_flight(req, stops=0):
    record = EvidenceRecord(type=EvidenceType.FLIGHT, provider="fixture", provider_reference="flight-1", amount=Decimal("500"), currency="USD", source_status=EvidenceSourceStatus.VERIFIED, normalized_data={"stops": stops})
...
def test_eval_passes_traceable_nonstop_proposal():
    ...
    proposal = ProposalDraft(request_id=req.request_id, status=ProposalStatus.READY_FOR_REVIEW, model_version="model-v1", evidence_pack_id=pack.evidence_pack_id, evidence_ids=[record.evidence_id], estimated_total=[MoneyAmount(amount=Decimal("500"), currency="USD", is_estimate=False)])
...
def test_eval_fails_when_proposal_references_missing_evidence():
    ...
    proposal = ProposalDraft(request_id=req.request_id, status=ProposalStatus.READY_FOR_REVIEW, model_version="model-v1", evidence_pack_id=pack.evidence_pack_id, evidence_ids=["ev_missing"])
    result = evaluate_proposal(req, pack, proposal)
...
def test_eval_fails_unverified_priced_evidence():
    req = request()
    record = EvidenceRecord(type=EvidenceType.HOTEL, provider="fixture", amount=Decimal("120"), currency="USD", source_status=EvidenceSourceStatus.UNVERIFIED, normalized_data={"price_basis": "per_night"})
...
def test_audit_bundle_links_exact_approved_snapshot():
    ...
    bundle = build_audit_bundle(req, pack, proposal, eval_result, approval=approval, final_output="Approved proposal", system_version="1.0.0", agent_release_id="travel-agent-test")
...
def test_audit_rejects_approval_for_modified_version():
    req = request()
    pack, record = verified_flight(req)
    proposal = ProposalDraft(request_id=req.request_id, version=1, status=ProposalStatus.READY_FOR_REVIEW, model_version="model-v1", evidence_pack_id=pack.evidence_pack_id, evidence_ids=[record.evidence_id])
    eval_result = evaluate_proposal(req, pack, proposal)
    approval = create_approval(proposal, eval_result, agent_id="agent-1")
    changed = proposal.model_copy(update={"version": 2, "summary": "changed"})
    changed_eval = evaluate_proposal(req, pack, changed)
    with pytest.raises(ValueError):
        build_audit_bundle(req, pack, changed, changed_eval, approval=approval, system_version="1.0.0", agent_release_id="travel-agent-test")
